...
# Remember that server.txt is the file containing a list of Server names to run this against
########################################################################
########################################################################
##		Author : Alex McAnnally   		    	      					##
##		  Date : 02/15/2019                                   			##
##		Update : 02/27/2019				      							##	 	
##         Description : PowerShell Systems Report          	      	##
##		   Provides information on current system status of servers		##
########################################################################
########################################################################

#region Variables and Arguments
$users = "TO_EMAIL1@EMAIL.com, TO_EMAIL2@EMAIL.com" # List of users to email your report to (separate by comma)
$fromemail = "FROM_EMAIL@EMAIL.com"
$server = "YOUR SMTP SERVER HERE" #enter your SMTP server DNS name / IP address here
$computers = Get-Content "D:\SystemsReport\Servers.txt" #This pulls from a text file the list of servers you want to run this report for
# Set free disk space threshold below in percent (default at 10%)
$thresholdspace = 15
[int]$EventNum = 3 #How many top Windows Event Logs you want to show in the report
[int]$ProccessNumToFetch = 10 #How many top Systems Processes with the high memory usage you want to show in the report
$ListOfAttachments = @()
$Report = @()
...
Function Create-PieChart() {
    ...
    foreach ($value in $args[0]) {
	    ...
	    $datapoint.AxisLabel = "(" + $value + " GB)" #Must be surrounded by a "(" and ")" otherwise it won't show the value on the chart
...
<html><head><title>PEER Systems Report</title>
<style type="text/css">
<!--
body {
...
foreach ($computer in $computers) {
	...
	| Select-Object SystemName, Name, @{n='Size (GB)';e={"{0:n2}" -f ($_.size/1gb)}}, @{n='FreeSpace (GB)';e={"{0:n2}" -f ($_.freespace/1gb)}}, @{n='PercentFree';e={"{0:n2}" -f ($_.freespace/$_.size*100)}} | ConvertTo-HTML -fragment
